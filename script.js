...
async function initModels() {
    try {
        ...
        ortSession = await ort.InferenceSession.create('./drowsiness_model.onnx');
...
async function startSystem() {
    ...
    try {
        ...
        // Hide placeholder immediately, but keep video visible until canvas draws
...
function stopSystem() {
    ...
    elements.videoFeed.style.opacity = "1"; // Show raw video if we restart
...
async function onFaceResults(results) {
    ...
    if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
        ...
        const isClosed = (leftEyeStatus === 0 && rightEyeStatus === 0);
...
// --- ONNX PREDICTION (With Square Crop & Contrast Boost) ---
async function predictEye(landmarks, indices, drawDebug) {
    ...
    tCtx.drawImage(elements.canvas, startX, startY, size, size, 0, 0, 64, 64);
    ...
    if(drawDebug) {
        ...
        ctx.drawImage(tempCanvas, debugX, debugY);
        ctx.fillStyle = "#fff";
        ctx.font = "12px Arial";
    ...
    const grayValues = new Float32Array(64 * 64);

    // 1. Convert to Gray & Find Min/Max
    ...
    // 2. Histogram Stretch & Normalize
    for (let i = 0; i < 64 * 64; i++) {
        let val = grayValues[i];
        if (maxVal > minVal) {
            val = (val - minVal) / (maxVal - minVal) * 255;
        }
        float32Data[i] = (val / 255.0 - 0.5) / 0.5;
    }
...
function processNoFaceDetected() {
    ...
    state.eyeHistory.push(0); // Assume safe
    if(state.eyeHistory.length > CONFIG.PERCLOS_WINDOW_SIZE) state.eyeHistory.shift();
...
function checkSafetyThresholds() {
    const now = Date.now();
    if (now - state.lastAlertTime < 3000) return; // 3s cooldown
...
function updateUI() {
    ...
    // 2. Closed Duration (Approx)
    // 30 FPS estimate for smooth display
